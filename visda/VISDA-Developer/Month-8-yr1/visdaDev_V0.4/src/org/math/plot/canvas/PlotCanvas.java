 ...
 * @author Yann RICHET
 * 
 * Modified by Jiajing Wang for center click feature on July 22, 2005;
 * Modified by Jiajing Wang for annotation feature on September 13, 2005;
 * Modified by Jiajing Wang for Changing the number of slices on the Axe on October 20, 2005
 * 
 */
public abstract class PlotCanvas extends JPanel implements MouseListener, MouseMotionListener, ComponentListener, BaseDependant, MouseWheelListener {
        ...
        if (ActionMode == SELCENTER) {     	
        	if (centers.size() != 0) {      		
        		...
        		for (int i=0; i<centers.size(); i++) {     			
        			String index = ""+(i+1);
        			draw.drawString(index, centers.get(i), 0, 0, 0);
        			draw.drawOval(centers.get(i));
        		}
        	}
        //} else {
        //	centers.clear();
    ...
    public int selectedPlotId;	// added by JJWang
    public ArrayList<double[]> centers = new ArrayList(); // added by JJWang
    ...
    /**
     * Change the number of slices on the specific Axe
     * Created by Jiajing Wang (October 20, 2005)
     *      
     * @param i		the index of Axe
     * @param sliceNum   the number of slices
     *  
     */
    public void setAxeSliceNum(int i, int sliceNum) {
        grid.setAxeSlices(i, sliceNum);
        repaint();
    }
}
