 ...
 * 	visda_v0.3 (09/29/2005):  Implement multiple data and multiple analysis view;
 * 								  add history log; Node can be deleted.
 *  visda_v0.4 (10/29/2005):  Separate the results into 'Clusters' and 'Levels' nodes.
 *  
 */
...
public class MainViewer extends JPanel {
     ...
     */
    //private synchronized void addAnalysisResult(DefaultMutableTreeNode node) {
    public void addClusterTopNode() {
    	// get the current data node
        DefaultMutableTreeNode curDataNode = (DefaultMutableTreeNode)expNode.getLastChild();
        // get the current analysis node
        DefaultMutableTreeNode curAnalysisNode = (DefaultMutableTreeNode)curDataNode.getLastChild();
          
        DefaultMutableTreeNode clusterTopNode = new DefaultNode("Clusters");
        
        // insert
        DefaultTreeModel treeModel = (DefaultTreeModel)tree.getModel();
        treeModel.insertNodeInto(clusterTopNode, curAnalysisNode, curAnalysisNode.getChildCount());
        TreeSelectionModel selModel = tree.getSelectionModel();
        TreePath treePath = new TreePath(clusterTopNode.getPath());
        selModel.setSelectionPath(treePath);
        tree.scrollPathToVisible(treePath);
        JScrollBar bar = this.treeScrollPane.getHorizontalScrollBar();
        if(bar != null)
     ...
     */
    //private synchronized void addAnalysisResult(DefaultMutableTreeNode node) {
    public void addLevelTopNode() {
    	// get the current data node
        DefaultMutableTreeNode curDataNode = (DefaultMutableTreeNode)expNode.getLastChild();
        // get the current analysis node
        DefaultMutableTreeNode curAnalysisNode = (DefaultMutableTreeNode)curDataNode.getLastChild();
          
        DefaultMutableTreeNode levelTopNode = new DefaultNode("Levels");
        
        // insert
        DefaultTreeModel treeModel = (DefaultTreeModel)tree.getModel();
        treeModel.insertNodeInto(levelTopNode, curAnalysisNode, curAnalysisNode.getChildCount());
        TreeSelectionModel selModel = tree.getSelectionModel();
        TreePath treePath = new TreePath(levelTopNode.getPath());
        selModel.setSelectionPath(treePath);
        tree.scrollPathToVisible(treePath);
        JScrollBar bar = this.treeScrollPane.getHorizontalScrollBar();
        if(bar != null)
     ...
     */
    //private synchronized void addAnalysisResult(DefaultMutableTreeNode node) {
    public void addLevelResult(DefaultMutableTreeNode node) {
        if (node == null) {
            return;
        }
        ...
        DefaultMutableTreeNode curDataNode = (DefaultMutableTreeNode)expNode.getLastChild(); 
        DefaultMutableTreeNode curAnalysisNode = (DefaultMutableTreeNode)curDataNode.getLastChild();
        DefaultMutableTreeNode curLevelTopNode = (DefaultMutableTreeNode)curAnalysisNode.getChildAt(3);//getLastChild();
        if ((String)curLevelTopNode.getUserObject() == "Levels") {
        	// update the node name
        	String nodeTitle = (String) node.getUserObject();
        	int index = curLevelTopNode.getChildCount();
        	//nodeTitle += " ("+(index-1)+")";
        	modifiedResult = true;
        	//node.setUserObject(nodeTitle);
        
        	DefaultTreeModel treeModel = (DefaultTreeModel)tree.getModel();
        	treeModel.insertNodeInto(node, curLevelTopNode, index);
        	TreeSelectionModel selModel = tree.getSelectionModel();
        	TreePath treePath = new TreePath(node.getPath());
        	selModel.setSelectionPath(treePath);
        	tree.scrollPathToVisible(treePath);
     ...
     */
    //private synchronized void addAnalysisResult(DefaultMutableTreeNode node) {
    public DefaultMutableTreeNode getClusterTopNode() {
        
        // get the current data and current analysis node
        DefaultMutableTreeNode curDataNode = (DefaultMutableTreeNode)expNode.getLastChild(); 
        DefaultMutableTreeNode curAnalysisNode = (DefaultMutableTreeNode)curDataNode.getLastChild();
        DefaultMutableTreeNode curClusterTopNode = (DefaultMutableTreeNode)curAnalysisNode.getChildAt(2);//getLastChild();
        if ((String)curClusterTopNode.getUserObject() == "Clusters") {
        	return curClusterTopNode;
        } else {
        	return null;
        }
        //addHistory("Analysis Result: "+nodeTitle);
