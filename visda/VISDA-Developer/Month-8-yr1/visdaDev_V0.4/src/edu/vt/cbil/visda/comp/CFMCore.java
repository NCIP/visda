 ...
 *  visda_v0.4 (10/29/2005): 	Implement MDL calculation; 
 * 
 */
public class CFMCore
{
	...
	/**
	 * indicate whether the EM results are valid (all values are real)
	 */
  ...
  // Declaration of the Native (C) function
	...
	public native int veSubEM(int n, int p, int k, double[] D, 
		...
		double[] mean1_x, double[] w0_t, double[] cov_mat, double[] Zjk, double[] mdl);
  	...
  	public CFMCore(int n, int p, int k)
    {
  		...
  		numClusters = k;
  		Zjk_up = one(n, 1.0);
  		// by default, initialize mdl to be negative so that the MDL calculation will NOT be performed.
		mdl = -1.0; 
    }
     ...
     */
  	public void veSubNew()
  	{
  		...
  		if (Common.isReal(W0_t) && (Common.isReal(Mean1_x)) && (Common.isReal(Cov_mat)) && (Common.isReal(Zjk))) {
  			isValid = true;
  		} else {
  			isValid = false;
  		}
