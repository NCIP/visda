...
import static org.math.array.DoubleArray.copy;
...
/**
 * The class is used to plot one single 2D projection combined the Posterior probabilities of samples
 * 
 * @author Jiajing Wang
 * @version visda_v0.4
 * <p>
 * Version Log:
 * 	<br>visda_v0.2 (08/30/2005):  The file is created and basic view tree is implemented.
 * 	<br>visda_v0.4 (10/29/2005):  Add the constructor for copy the class. 
 * 
 */
public class Plot2DViewer extends JPanel implements MouseListener
{
    		   ...
    		   ExperimentData expdata)
	{
		...
		center2D[0] = center2D[0] / dotNum;
		center2D[1] = center2D[1] / dotNum; 
	}
	
	
	/**
	 * Generate a new class which is copied from an existed one
	 * @param p		the class which the new one will copy from
	 * 
	 */
	public Plot2DViewer(Plot2DViewer p)
	{
		this.plotName = p.getPlotName();
		this.id = ++p.id;
		this.expData = p.expData;
		this.displayThreshold = p.displayThreshold;
		this.annotateThreshold = p.annotateThreshold;
		this.Zjk = copy(p.Zjk);
		
		this.plotPanel = new Plot2DPanel();
		for (int i=0; i<p.plotPanel.plotCanvas.getPlots().length; i++) {
			this.plotPanel.addPlot(p.plotPanel.plotCanvas.getPlot(i));
		}
		this.plotPanel.removePlotToolBar();
		this.plotPanel.plotCanvas.addMouseListener(this);
		add(this.plotPanel);
		this.center2D = copy(p.center2D);
	}
	
	
	public void setPlotName(String s) {
		plotName = s;
	}
	...
	public void addCenterDot() {	
		double[][] d = new double[1][2];
		d[0] = center2D;
		plotPanel.addPlot("SCATTER", "CENTER", Color.MAGENTA, d);
	}
	
	public double[] getCenterDot() {
		return center2D;	
	}
}
