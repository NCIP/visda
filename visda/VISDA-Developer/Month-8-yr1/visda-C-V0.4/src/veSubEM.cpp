...
int veSubEM(matrix *D, matrix *mean0_x, vector *w0, double vv, double error, vector *Zjk_up, //input
			matrix *mean1_x, vector *w0_t, matrix *cov_mat, matrix *Zjk, double *mdl)  //output
{
	int k0, kc, n, p;
	int i, j, k, u, s;
	matrix *Var0;
	...
	// If mdl>0, then calculate the MDL value; 
	// if mdl<0, then skip.
	//		MDL(K) = -ln(L)+K*N*ln(N)/(2N-K)
	if (*mdl > 0) {
		double bias = 0;
		double sum_Zjkup = 0;
		for (i=0; i<Zjk_up->l; i++) {
			bias += (Zjk_up->pr)[i] * log((Fx.pr)[i]);
			sum_Zjkup += (Zjk_up->pr)[i] ;
		}
		if ((2 * sum_Zjkup) > (6 * k0 - 1)) {
			*mdl = -bias + sum_Zjkup * (6 * k0 - 1 ) * log(sum_Zjkup) / (2 * sum_Zjkup - (6 * k0 - 1));
		} else {
			*mdl = 1e+8;
		}
	}
