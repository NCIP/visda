...
//	File:			veSNR.cpp
//	Project:		VISDA
//	Description:	for supervised feature selection using signal-to-noise ratio (SNR)
//	Author:			Jiajing Wang
...
int veSNR(matrix *data, int *label, int dim_num, 
		  int *top_label, vector *snr_sorted, vector *mean_sorted, vector *var_sorted)
{
	...
	vnew(mean_sorted, n*num_class);
	for (i=0; i<num_class; i++) {
		int t = i*n;
		for (j=0; j<n; j++) {
			int new_id = new_order[j];
			mean_sorted->pr[t+j] = (data_mean+i)->pr[new_id];
		}
	}

	vnew(var_sorted, n*num_class);
	for (i=0; i<num_class; i++) {
		int t = i*n;
		for (j=0; j<n; j++) {
			int new_id = new_order[j];
			var_sorted->pr[t+j] = (variance+i)->pr[new_id];
		}
	}
